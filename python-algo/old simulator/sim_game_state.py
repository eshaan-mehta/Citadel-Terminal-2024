...
class SimGameState:
    ...
    def __init__(self, config, serialized_string, test):
        ...
        self.game_map = SimGameMap(self.config)
        self._shortest_path_finder = SimShortestPathFinder()
    ...
    def __parse_state(self, state_line, test):
        ...
        # add test units to the game state units
        p1units = [ a + b for a, b in zip(state["p1Units"], test["p1Units"]) ] 
        p2units = [ a + b for a, b in zip(state["p2Units"], test["p2Units"]) ]
    ...
    def get_target(self, attacking_unit):
        ...
        if not isinstance(attacking_unit, SimGameUnit):
